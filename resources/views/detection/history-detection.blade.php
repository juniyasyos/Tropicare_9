    ...
    <div class="md:p-4 md:ml-64">
        <div class="bg-white h-auto rounded-lg">
                    ...
                    <div class="text-sm breadcrumbs hidden md:block lg:block">
                        <ul>
                            <li>
                                ...
                                <a href="{{ route('dashboard') }}" class="font-semibold text-lg underline">
                                    Rekapitulasi
                                </a>
                            </li>
                        </ul>
            ...
            <div class="container mx-auto p-6 mt-[-20px]">
                <div class="fixed flex justify-end w-9/12 z-30 top-7 right-0">
                    @if (session('error'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                            ...
                            role="alert" class="alert alert-error bg-red-500 text-white p-4 rounded-md shadow-md">
                    ...
                    @if (session('data_updated'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                            ...
                            x-transition:leave="transition ease-in duration-500 transform"
                            x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full"
                            role="alert" class="alert alert-success bg-green-500 text-white p-4 rounded-md shadow-md">
                            <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-6 w-6"
                                fill="none" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex bg-white mt-5 justify-between items-center dark:bg-gray-800">
                ...
                <div class="grid gap-4 my-4 grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
                    @if ($detections->isEmpty())
                        <div class="flex flex-col items-center justify-center col-span-full">
                            <svg xmlns="http://www.w3.org/2000/svg" width="200" height="200"
                                ...
                                <g>
                                    <line x1="83.843" x2="72.652" y1="213.182" y2="188.61"
                                        fill="none" stroke="#4A486A" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-miterlimit="10" stroke-width="10"></line>
                                    <line x1="100.224" x2="106.915" y1="205.721" y2="187.84"
                                        fill="none" stroke="#4A486A" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-miterlimit="10" stroke-width="10"></line>
                                    <line x1="67.462" x2="49.581" y1="220.643" y2="213.952"
                                        fill="none" stroke="#4A486A" stroke-linecap="round"
                                        stroke-linejoin="round" stroke-miterlimit="10" stroke-width="10"></line>
                                </g>
                                <g>
                    ...
                    @else
                        ...
                        @foreach ($detections as $index => $detection)
                            <div
                                ...
                                <div class="p-5">
                                    <a href="#">
                                        <h5
                                            class="mb-2 text-md md:text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                                    ...
                                    <div class="w-full flex justify-end">
                                        <button data-title="{{ $detection->ResultDetection }}"
                                            data-date="{{ $detection->created_at->format('d M Y') }}"
                                            data-image="{{ asset('storage/' . $detection->PlantPhoto) }}"
                                            data-id="{{ $detection->DetectionID) }}"
                                            class="modal-open inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                            aria-haspopup="dialog" aria-controls="modal" aria-expanded="false">
                                            Detail
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div id="modal"
                            ...
                            aria-hidden="true">
                            <div id="modal-box"
                                class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-11/12 md:w-1/2 p-5">
                                <h3 id="modal-title" class="font-bold text-lg mb-5"></h3>
                                <img id="modal-image" class="rounded-lg shadow-lg w-full h-auto mb-4"
                                    alt="Plant Image">
                                <p id="modal-date" class="text-sm text-gray-700 dark:text-gray-400"></p>
                                <div class="flex justify-end space-x-4 mt-4">
                                    <button id="modal-close" class="btn w-3/12 btn-outline btn-error">Keluar</button>
                                    <button id="delete-button" class="btn w-3/12 text-white btn-error">Hapus</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="delete-modal"
                            ...
                            <div id="delete-modal-box"
                                class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-11/12 md:w-1/2 p-5">
                                <h3 class="font-bold text-lg mb-5">Konfirmasi Hapus</h3>
                                <p id="delete-modal-message" class="text-sm text-gray-700 dark:text-gray-400 mb-4">
                                ...
                                <div class="flex justify-end space-x-4 mt-4">
                                    <form id="delete-form" method="POST" action="">
                                        ...
                                        <button id="confirm-delete" type="submit"
                                            class="btn w-3/12 text-white btn-error">Hapus</button>
                                    </form>
                                    <button id="cancel-delete" class="btn w-3/12 btn-outline btn-error">Batal</button>
                                </div>
                            </div>
                        </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const closeModal = () => {
                        ...
                        document.querySelector('.modal-open[aria-expanded="true"]').setAttribute('aria-expanded',
                            'false');
                    };

                    // Menampilkan modal konfirmasi penghapusan ketika tombol hapus diklik
                    deleteButton.addEventListener('click', function() {
                        deleteModal.classList.remove('hidden');
                        deleteModal.setAttribute('aria-hidden', 'false');
                    });

                    // Menyembunyikan modal konfirmasi penghapusan ketika tombol batal diklik
                    cancelButton.addEventListener('click', function() {
                        deleteModal.classList.add('hidden');
                        deleteModal.setAttribute('aria-hidden', 'true');
                    });

                    // Mengatur action form dan mengirim saat penghapusan dikonfirmasi
                    confirmDeleteButton.addEventListener('click', function() {
                        // Mengambil id data yang akan dihapus
                        const id = deleteButton.dataset.id;


                        // Mengatur action form dengan endpoint hapus data
                        deleteForm.action = `/detections/${id}`;
                        // Mengirim form
                        deleteForm.submit();
                    });

                    // Menampilkan modal saat tombol modal-open diklik
                    document.querySelectorAll('.modal-open').forEach(button => {
                        button.addEventListener('click', () => {
                            ...
                            modalBox.querySelector('#modal-date').textContent = date;
                            modalBox.querySelector('#modal-image').src = image;

                            modal.classList.remove('hidden');
                            modal.setAttribute('aria-hidden', 'false');
                            button.setAttribute('aria-expanded', 'true');

                            // Mengatur data id pada tombol hapus
                            deleteButton.dataset.id = id;
                        });
                    });

                    // Menutup modal saat tombol modal-close diklik
