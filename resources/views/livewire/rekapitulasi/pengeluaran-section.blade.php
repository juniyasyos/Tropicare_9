<div class="h-auto mt-[-15px] rounded-sm">
    <div class="mt-4 p-6 bg-white flex items-center space-x-6">
        <div>
            ...
            <p class="font-inter text-[20px] text-gray-700 font-bold">Rp{{ number_format($expenditures) }}</p>
        </div>
    </div>
        ...
        <div>
            @if ($expenditure->isEmpty())
                ...
                <div class="flex justify-center mt-5">
                    <button class="btn btn-success text-white w-1/2" onclick="modalAddExpenditure.showModal()">Tambah
                        Pengeluaran</button>
                </div>
            @else
                    ...
                    <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                        <thead class="bg-gray-800 text-white">
                            <tr>
                                ...
                                <th class="w-2/12 py-3 px-4 font-semibold text-sm">Nama Pengeluaran</th>
                                <th class="w-3/12 py-3 px-4 font-semibold text-sm">Tanggal</th>
                                <th class="w-1/12 py-3 px-4 font-semibold text-sm">Deskripsi</th>
                                <th class="w-2/12 py-3 px-4 font-semibold text-sm">Total</th>
                            </tr>
                        </thead>
                        @foreach ($expenditure as $index => $expendse)
                            <tr onclick="showExpenditureEdit({{ json_encode($expendse) }})"
                                class="bg-gray-100 border-b hover:bg-blue-50 transition duration-300 cursor-pointer">
                                <td class="py-3 px-4 text-center">{{ $index + 1 }}</td>
                                <td class="py-3 px-4 text-center">{{ $expendse->NameExpenditure }}</td>
                                <td class="py-3 px-4 text-center">
                                    {{ \Carbon\Carbon::parse($expendse->ExpenditureDate)->translatedFormat('j F Y') }}
                                </td>
                                <td class="py-3">{{ $expendse->Description }}</td>
                                <td class="py-3 px-4 text-center">Rp{{ number_format($expendse->Amount) }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                ...
                <div class="md:-ml-[65px] md:hidden space-y-3">
                    @foreach ($expenditure as $expendse)
                        <div onclick="showExpenditureEdit({{ json_encode($expendse) }})"
                            ...
                            <div>
                                <p class="font-normal text-[12px] font-inter">
                                    {{ \Carbon\Carbon::parse($expendse->ExpeditureDate)->translatedFormat('j F Y') }}
                                </p>
                                <h5 class="text-xl font-semibold text-gray-700 tracking-tight font-inter">
                                    Rp{{ number_format($expendse->Amount) }}
                                </h5>
                            </div>
                            <div class="absolute bottom-3 right-2">
                                <h3 class="text-sm">{{ $expendse->NameExpenditure }}</h3>
                            </div>
                        </div>
                    @endforeach
                </div>
                @include('livewire.rekapitulasi.partials.modal-detail-expenditure')
            @endif
        </div>
    </div>
    @include('livewire.rekapitulasi.partials.modal-add-expenditure')
</div>
